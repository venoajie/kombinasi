...
#			tahun=int(str((instsB['instrument_name'])[-2:]))+int(str((datetime.strptime( (instsB['instrument_name'])[-5:][:3], '%b' ).month)))
			...
			# masalah:
                            # tetap boleh pesan normal selama prose cutloss? NGGAK. PESAN HANYA BOLEH BILA SALDO = 0/10-->kenyataanya masih bisa
							# penggandaan qty amat beresiko, efek berantainya maks 2-3 kali kesalahan sj sblm meledak. lihat atas
                            ...
                            # bagaimana kalau 2 sisi yang harus di cutloss? ada resiko, dibatasi dengan hanya boleh pesan setelah 2 kali beli selesai-->sisi yang banyak menang
                                #memesan tanpa henti karena selurh kondisi cutloss terpenuhi?
								#kalau satu sisi cutloss dah 0, boleh pesan buat sisi itu?
			...
			cutloss_qty_sell_double = 0
			cutloss_qty_buy_double = 0
			...
			cutloss_qty_sell_double = abs(hold_sell_qty_cl) * 2

			cutloss_qty_buy_double = abs(hold_buy_qty_cl) * 2
		...
		#A. Kapan mulai cutloss: threshold = margin/5
			...
			cutloss = 1 if (cutloss_prc_sell_test == 1 or cutloss_prc_buy_test == 1 ) else 0
			...
			if cutloss_prc_sell_test == 1 and hold_buy_qty_cl >0  and (hold_buy_qty !=0 or hold_sell_qty !=0) and hold_buy_qty_cl/(1 if abs(hold_sell_qty_cl) == 0 else abs(hold_sell_qty_cl)
				)>=2 and openOrders_sell_qty ==0:

				if openOrders_buy_qty_limit == qty:                                                   
			...
			elif cutloss_prc_buy_test == 1 and hold_sell_qty_cl <0  and (
				hold_buy_qty !=0 or hold_sell_qty !=0) and abs(hold_sell_qty_cl)/(1 if hold_buy_qty_cl == 0 else hold_buy_qty_cl
				)>=2 and openOrders_buy_qty ==0:

				if abs(openOrders_sell_qty_limit) == qty:                                                   
			...
			print(sub_name,fut,'cutloss_prc_sell_test',cutloss_prc_sell_test,'cutloss_prc_sell',cutloss_prc_sell)
			print(sub_name,fut,'cutloss_prc_buy_test',cutloss_prc_buy_test,'cutloss_prc_buy',cutloss_prc_buy)
			...
			for i in range(max(nbids, nasks)):
				...
				if  perp_test== 1 or fut_test==1:
					...
					filled_time = ((time_now)-  time.mktime(datetime.strptime(
                                            [o['receivedTime']for o in openOrders_CF][0
                                            ],'%Y-%m-%dT%H:%M:%S.%fZ').timetuple())*1000
                                            ) if deri_test==0 else (time_now/1000- max(
                                            filledOrders_sell_lastTime_max,filledOrders_sell_lastTime_max)/1000)

					print(sub_name,fut,'filled_time',filled_time)
				...
				print(sub_name,fut,'open_time',open_time, 'openOrders_sell_qty_limit',openOrders_sell_qty_limit)
				if open_time >180:# and api ==True:
					if deri_test == 1 :
						...
						if  perp_test==1 and openOrders_buy_oid_stopLimit  !=0:                                                   
							...
							if openOrders_buy_qty_limit != 0:                                                   
								client_trading.private_cancel_get(openOrders_buy_oid_limit)

							elif abs(openOrders_sell_qty_limit) != 0:                                                   
								client_trading.private_cancel_get(openOrders_sell_oid_limit)
				...
				if place_bids:
					...
					if deri_test == 1:
						...
						if   perp_test==1 :
							...
							if  (openOrders_buy_qty_stop <qty and openOrders_sell_prc_limit >1 and abs(hold_sell_qty_cl) <qty) and cutloss == 0:
								...
								print(sub_name,fut,'fut_test==1','AAA','stop_prc_perp',stop_prc_perp)
							...
							#NORMAL, bila limit diekseskusi, tetapi stop_limit kesangkut.
							...
							elif filledOpenOrders_qty_totalNet<0 and openOrders_sell_qty_stop ==0 and cutloss == 0:                                                   
								print(sub_name,fut,'fut_test==1','BBB','filledOrders_sell_prc',filledOrders_sell_prc,'(delta_prc *abs( filledOrders_sell_prc)',delta_prc *abs( filledOrders_sell_prc))
								...
								client_trading.private_buy_get(
									instrument_name =fut,
									 ...
									 type            ='limit',
									 price           =min(
										 filledOrders_sell_prc if hold_avg_prc <1 else hold_avg_prc ,abs(
										filledOrders_sell_prc)) -(delta_prc *abs( filledOrders_sell_prc)),
                                    post_only       ='true',
									amount          =qty
									)	
								#print(fut,'fut_test==1','BBB')


							#memastikan hanya ada 1 order setiap waktu
							elif  openOrders_buy_qty_stop >qty or (cutloss == 1 and openOrders_buy_oid_stopLimit !=0):
								print('openOrders_buy_oid_stopLimit',openOrders_buy_oid_stopLimit)

								client_trading.private_cancel_get(openOrders_buy_oid_stopLimit)

							#CUTLOSS buy, perp jual laba untuk menutupi kerugian fut
							elif cutloss_prc_buy >1:                                                   
								print(sub_name,fut,'fut_test==1','CCC','cutloss_prc_buy',cutloss_prc_buy)
							...
							elif cutloss_prc_sell >1 and openOrders_buy_qty ==0:                                                   
								print(sub_name,fut,'fut_test==1','DDD')
                            ...
                            #open_time >600, beri kesempatan avg down 1 x setelah 10 menit
							
							if  (hold_buy_qty == 0 ):
								#(hold_buy_qty_cl == (qty ) and cutloss == 1) or                                                    
								print(sub_name,fut,'fut_test==1','EEE','hold_buy_qty_cl',hold_buy_qty_cl,'cutloss',cutloss)
								client_trading.private_buy_get(
									instrument_name=fut,
									amount=qty,
									price=bid_prc ,
							...
							elif  openOrders_buy_prc_limit >qty or (cutloss == 1 and openOrders_buy_oid_limit !=0):
								print('openOrders_buy_oid_limit',openOrders_buy_oid_limit)
								client_trading.private_cancel_get(openOrders_buy_oid_limit)

							#CUTLOSS sell, beli 2 kali qty perp pada harga mkt
                            # filled_time >600, memastikan avg down tidak seketika di 1 harga
							# and hold_buy_qty_cl < (qty*2) and abs(hold_sell_qty_cl) == qty; memastikan cutloss dengan komposisi 10 20
							
							elif cutloss ==1 and hold_buy_qty_cl < (qty*2) and abs(hold_sell_qty_cl) == qty and filled_time >600:                                                   
								print(sub_name,fut,'fut_test==1','FFF','cl/abs',hold_buy_qty_cl/abs(hold_sell_qty_cl)<2 )
								client_trading.private_buy_get(
									instrument_name =fut,
									reduce_only     ='false',
									type            ='limit',
				...
				if place_asks:
					...
					if deri_test == 1:
						...
						print(sub_name,fut,'stop_prc_fut',stop_prc_fut,'openOrders_buy_prc_limit',openOrders_buy_prc_limit)
						prc=   openOrders_buy_prc_limit+(delta_prc * openOrders_buy_prc_limit)
                       
						if   fut_test==1 : 
							#normal,stop limit seketika setelah eksekusi order 
							if (openOrders_sell_qty_stop < qty and openOrders_buy_prc_limit >1 and hold_buy_qty_cl<qty) and cutloss == 0:
								print(sub_name,fut,'fut_test==1','111')
							...
							elif  filledOpenOrders_qty_totalNet>0 and openOrders_buy_qty_stop ==0 and cutloss == 0:													
								print(sub_name,fut,'fut_test==1','222','hold_avg_prc',hold_avg_prc,'filledOrders_buy_prc',filledOrders_buy_prc,max(hold_avg_prc,filledOrders_buy_prc))

								client_trading.private_sell_get(
									instrument_name=fut,
									reduce_only='false',
									type='limit',
									price=max(hold_avg_prc,filledOrders_buy_prc)+(delta_prc*filledOrders_buy_prc),
									post_only='true',
									amount=qty)	

						
							#memastikan hanya ada 1 order setiap waktu
							elif  abs(openOrders_sell_qty_stop) >qty or (cutloss == 1 and openOrders_sell_oid_stopLimit !=0):
							...
							#cutloss sell, fut jual laba untuk menutupi kerugian perp
								...
								print(sub_name,fut,'fut_test==1','333')
							...
							elif abs(cutloss_prc_buy) >1 and openOrders_sell_qty ==0:                                                   
								print(sub_name,fut,'fut_test==1','444')
							...
							if hold_sell_qty == 0 :
								  #(abs(hold_sell_qty_cl) ==(qty) and cutloss == 1) or 							
								print(sub_name,fut,'fut_test==1','555','hold_sell_qty_cl',hold_sell_qty_cl)
							...
							elif  openOrders_sell_prc_limit >qty or (cutloss == 1 and openOrders_sell_oid_limit !=0):
								print('openOrders_sell_oid_limit',openOrders_sell_oid_limit)

								client_trading.private_cancel_get(openOrders_sell_oid_limit)

							#CUTLOSS buy, beli 2 kali qty fut pada harga mkt 
                            # filled_time >600, memastikan avg down tidak seketika di 1 harga
							# and abs(hold_sell_qty_cl) < (qty*2) and hold_buy_qty_cl == qty; memastikan cutloss dengan komposisi 10 20
							if  cutloss ==1 and abs(hold_sell_qty_cl) < (qty*2) and hold_buy_qty_cl == qty and filled_time >600:							
								print(sub_name,fut,'fut_test==1','666','abs(hold_sell_qty_cl)',abs(hold_sell_qty_cl))
