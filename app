...
class MarketMaker(object):
	...
	def place_orders(self):
		...
		for fut in deriCF:
			...
			filledOrders_sell_prc   = ( [ o['price'] for o in [ o for o in last_prc_CF if o[
                                            'side'] == 'sell' and  o['symbol']==fut  ] ] )

			filledOrders_sell_prc_CF= 0 if filledOrders_sell_prc == [] else filledOrders_sell_prc [0]

			filledOrders_buy_prc    = ( [ o['price'] for o in [ o for o in last_prc_CF if o[
...
#			print(fut,'openOrders_buy_qty ',openOrders_buy_qty ,'openOrders_sell_qty ',openOrders_sell_qty,'openOrders_qty_Net',openOrders_qty_Net)
			...
			try:
				openOrders_buy_oid_limit   = 0 if openOrders_buy_time_api ==0 else ( [ o['order_id'
                                                    ] for o in [ o for o in openOrders if o[
                                                    'creation_timestamp'] == openOrders_buy_time_api  and  o[
                                                        'order_type']=='limit' ] ] [0])                           
			except:
			...
			try:
				openOrders_sell_oid_limit  = 0 if openOrders_sell_time_api ==0 else( [ o['order_id'
                                                 ] for o in [ o for o in openOrders if o[
			...
			try:
				openOrders_buy_oid_stopLimit   = 0 if openOrders_buy_time_api ==0 else (
                                    [ o['order_id'] for o in [ o for o in openOrders if o[
                                                    'direction'] == 'buy'  and o['api']==True and o[
                                                    'triggered'] == False  and  o[
                                                        'order_type']=='stop_limit'  ] ] [0])                           
			except:
				openOrders_buy_oid_stopLimit = 0

			try:
				openOrders_sell_prc_limit  = 0 if openOrders_sell_time_api ==0 else( [ o['price'
                                                 ] for o in [ o for o in openOrders if o[
                                                'creation_timestamp'] ==openOrders_sell_time_api  and  o[
                                                    'order_type']=='limit']] [0])
			except:
			...
			try:
				openOrders_sell_oid_stopLimit  = 0 if openOrders_sell_time_api ==0 else( [ o['order_id'
                                                 ] for o in [ o for o in openOrders if  o[
